	...
	static function set($name,$value = null){  
		...
		*/
		if(!$value && (is_array($name) || is_object($name))){
			$name = (array)$name;
			foreach($name as $k=>$v){
				$_SESSION[$k] = static::init()->encode(ForCookieSessionArrayValue::cookie($v));
			} 
			return $name;
		}
		if($value != null){
			$value = static::init()->encode(ForCookieSessionArrayValue::cookie($value));
		}
	...
	static function get($name = null){
		if(!$name && $_SESSION) { 
			foreach($_SESSION as $k=>$v){ 
				$data[$k] = ForCookieSessionArrayValue::re_cookie(static::init()->decode($v));
			} 
			return  (object)$data;
		} 
		...
		$value = $_SESSION[$name];
		if($value)
			return ForCookieSessionArrayValue::re_cookie(static::init()->decode($value));		 
	} 
	/**
	* 删除SESSION,一个或多个或所有
	*
